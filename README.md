...
專案保留一個可調參數的迴歸模擬器，用來確認模型、殘差與離群值排序的基礎流程；主要案例則放在中部空氣品質指標 (Air Quality Index, AQI) / PM2.5 等數值資料，讓分析結果可以對應到空污觀測、戶外活動安排與營運風險提醒等實際需求。
...
| 資料來源 | 優先使用環境部 `AQX_P_488` 歷史 AQI；Kaggle Taiwan AQI 作為可重現歷史參考 |
...
- **線性模型擬合**：迴歸模擬器與 AQI 案例皆可選擇 OLS、Ridge、Lasso、ElasticNet；若環境缺少 scikit-learn 則以 NumPy least-squares 作為 fallback。
...
- **Data Source**：整合目前使用資料、Lab 合成資料邊界、Kaggle 歷史參考與環境部官方來源。
- **替代研究主題**：提供 [docs/topic_alternatives.md](./docs/topic_alternatives.md)，整理生活化與中部公開資料題材。
- **UI/UX Prompt**：提供可重用的 AI UI/UX 開發提示詞，避免後續修改只停留在表面 CSS。
- **線性回歸學習任務**：提供 [docs/linear_regression_learning_task.md](./docs/linear_regression_learning_task.md)，用初學者視角拆解參數、指標與 AQI 案例。

---

## UI/UX Prompt 使用方式

[docs/uiux_ai_prompt.md](./docs/uiux_ai_prompt.md) 是專案文件，不會自動觸發 Codex 或其他 AI 工具。

使用時可以在對話中明確指定：

```text
請依照 docs/uiux_ai_prompt.md 的準則，檢查並改善這個 Streamlit app 的 UI/UX。
優先處理淺色模式文字對比、深色模式殘留、圖表可讀性與非技術使用者的白話解釋。
```

如果希望它像 Codex skill 一樣被自動套用，下一步需要另外建立 skill，並把這份 prompt 改寫成 `SKILL.md`。

---

## Data Source

AQI 頁目前使用 `data/central_taiwan_aqi_sample.csv`，共有 82,034 筆，範圍包含 Taichung City、Changhua County 與 Nantou County。這份資料用來支援實際空氣品質案例的回歸建模、殘差排序與異常觀測判讀。

Lab 頁不使用老師課堂資料、Kaggle、環境部或任何 CSV。Lab 的資料由 `app.py` 依照 `y = a*x + b + noise` 在記憶體即時產生；左側的樣本數、斜率、截距、雜訊變異數與隨機種子就是資料生成條件。

本專案保留 Kaggle 的 [Taiwan Air Quality Index Data 2016~2024](https://www.kaggle.com/datasets/taweilo/taiwan-air-quality-data-20162024) 作為可重現歷史資料參考。它不是目前 app 實際讀取的檔案。
...
24 筆微型 demo 已移除，不再作為 fallback。若 `central_taiwan_aqi_sample.csv` 不存在，應更換足夠大的公開資料或研究主題。
...
- **Data Understanding**：說明目前 AQI CSV、Lab 合成資料邊界、Kaggle 與環境部 AQI 來源。
- **Data Preparation**：整理欄位、數值轉換、缺值處理與中部/台中/彰化/南投篩選。
- **Modeling**：以 OLS、Ridge、Lasso、ElasticNet 建立線性模型比較。
...
└── docs/
    ...
    ├── linear_regression_learning_task.md
    ├── topic_alternatives.md
    └── uiux_ai_prompt.md
```

`data/central_taiwan_aqi_sample.csv` 是 AQI 頁目前實際使用資料。`data/CentraArea_Data.csv` 保留為本機歷史資料參考。微型 demo 已移除，不再作為 fallback。若未來要支援外部 CSV，應先定義欄位 schema 與清理規則，再開放上傳。

...
- **線性模型擬合**：AQI 案例可比較 OLS、Ridge、Lasso、ElasticNet；若環境缺少 scikit-learn 則以 NumPy least-squares 作為 fallback。
- **離群值偵測**：計算每筆資料的 residual 與 absolute residual，列出距離迴歸線最遠的前 10 筆觀測。
- **互動視覺化**：圖表同時呈現生成資料、真實線、迴歸線與 Top 10 outliers。
- **中彰 AQI 案例**：目前實際執行使用內建中彰 AQI sample，篩選台中市、彰化縣測站資料進行 AQI 預測。
...
- **資料與報告下載**：使用者可下載內建 sample CSV；CRISP-DM markdown 報告會依目前中英文模式輸出。
...
目前 app 實際執行使用的是 `data/central_taiwan_aqi_sample.csv` 內建 sample。這份資料用於課堂與本機展示，欄位邏輯對齊台灣空氣品質開放資料，但不是即時 API 抓取。

本專案建議以 Kaggle 的 [Taiwan Air Quality Index Data 2016~2024](https://www.kaggle.com/datasets/taweilo/taiwan-air-quality-data-20162024) 作為可重現練習資料參考。它已整理成適合機器學習練習的 AQI、污染物與時間資料，適合做 regression、multivariate analysis 與 outlier detection。
...
`data/central_taiwan_aqi_sample.csv` 是目前 app 預設實際使用資料；Kaggle 與 MOENV 則作為資料來源參考與未來替代方向。若未來要支援外部 CSV，應先定義欄位 schema 與清理規則，再開放上傳。
